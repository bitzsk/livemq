	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		if(in.readableBytes() > 0) {
			...
			if(in.readableBytes() < PACKET_LENGTH) {
				logger.warn("decoder 读取到的字节流小于数据的最小长度 ["+ PACKET_LENGTH +"]");
				return;
			}
			...
			logger.info("decoder count:{}", ++count);
			...
			if(in.readableBytes() < len) {
				logger.warn("decoder 读取到的字节流长度为 ["+ in.readableBytes() +"], 而消息的总长度为 ["+ len +"], 等待继续读取，同时将 readerIndex 设置为 ["+ beginIndex +"]");
